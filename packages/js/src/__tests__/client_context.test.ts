...
describe("Client context should be able to encode as expected", () => {
  ...
  it("the server context should trigger a header change update", async () => {
    ...
    edge
      .received(2)
      .contextChange(
        "city=istanbul,country=turkey,device=desktop,musical styles=deep%2Cpsychedelic,platform=macos,session=VirtualBurningMan,userkey=DJElif,version=4.3.2",
      );
  });
  ...
  it("the server context should close the current edge if it has been told to reset edge", async () => {
    ...
    expect(serverContext.getString("joy", "loopypro")).toBe("loopypro");
    expect(serverContext.getJson("joy", { x: 1 })).toEqual({ x: 1 });
    expect(serverContext.getRawJson("joy", "raw-json")).toBe("raw-json");
  });
