...
describe('Catch and release should hold and then release feature changes', () => {
  ...
  it('should trigger when a new feature arrives', async () => {
    ...
    internalRepo.notify(SSEResultState.Feature,
      { id: '1', key: 'orange', version: 1, type: FeatureValueType.String } as FeatureState
    );
    internalRepo.notify(SSEResultState.Feature,
      { id: '1', key: 'orange', version: 2, type: FeatureValueType.String, value: 'lemon' } as FeatureState
    );
  ...
  it('should enable me to turn on catch and no changes should flow and then i can release', async () => {
    ...
    internalRepo.notify(SSEResultState.Feature, { id: '1', key: 'banana', version: 2,
    ...
    internalRepo.notify(SSEResultState.Feature, { id: '1', key: 'banana', version: 3,
      type: FeatureValueType.Boolean, value: false } as FeatureState);

    expect(postNewTrigger).to.eq(2);
    expect(bananaTrigger).to.eq(1);

    internalRepo.notify(SSEResultState.Feature, { id: '3', key: 'apricot', version: 1, type: FeatureValueType.Boolean, value: false } as FeatureState);
    ...
    const features2 = [
      { id: '1', key: 'banana', version: 4, type: FeatureValueType.Boolean, value: true } as FeatureState,
    ];
  ...
  it('should allow be to turn on catch & release mode, get updated, remove the trigger and then not be notified with the trigger removed', async () => {
    ...
    const features2 = [
      { id: '1', key: 'banana', version: 2, type: FeatureValueType.Boolean, value: false } as FeatureState,
    ];
