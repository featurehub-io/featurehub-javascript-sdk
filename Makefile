.PHONY: image image-backend image-tests start-backend start-backend-qa test-server test-server-tags test-server-qa test-server-qa-tags

image:
	@echo "Building Docker image for JavaScript SDK..."
	docker build -t featurehub/js-sdk:latest .
...
image-tests:
	...
	docker build -f Dockerfile.tests -t featurehub/js-sdk-cukes:1.0 .
...
_run-test:
	@$(TEST_ENV) && pnpm --filter './examples/todo-server-tests' run test $(if $(TAGS),-- --tags $(TAGS))
