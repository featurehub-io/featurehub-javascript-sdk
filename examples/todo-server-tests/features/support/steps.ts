...
Given("I wipe my list of todos", async function (this: { user: string }) {
...
Given(
  "I set the flag {string} to {string}",
  async function (featureKey: string, featureValue: string) {
...
When(
  "I attempt to update feature {string} to boolean value {string}",
  async function (featureKey: string, value: boolean) {
    await this["updateFeatureOnlyValue"](featureKey, value);
  },
);

When(
  "I attempt to update feature {string} to number value {string}",
  async function (featureKey: string, value: number) {
    await this["updateFeatureOnlyValue"](featureKey, value);
  },
);

When(
  "I attempt to update feature {string} to string value {string}",
  async function (featureKey: string, value: string) {
    await this["updateFeatureOnlyValue"](featureKey, value);
  },
);
Then(
  /^feature (.*) is (locked|unlocked) and "([^"]*)"$/,
  async function (featureName: string, lockedStatus: string, value: string) {
...
Given(
  /^the feature (.*) is (unlocked|locked)$/,
  async function (featureName: string, lockedStatus: string) {
    const locked = lockedStatus === "locked";
    const ctx = await Config.fhConfig.newContext().build();

    // has to be longer than the polling interval
    const timeout = process.env["FEATUREHUB_POLLING_INTERVAL"]
      ? parseInt(process.env["FEATUREHUB_POLLING_INTERVAL"]) + 2000
      : 4000;
